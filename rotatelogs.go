...
import (
	...
	"github.com/GitHub121380/file-rotatelogs/internal/fileutil"
...
func New(basename string, options ...Option) (*RotateLogs, error) {
	...
	for _, o := range options {
		...
		case optkeyRotationRegRule:
			regRule = o.Value().(string)
		case optkeyHandler:
	...
	if regRule == "" {
		regRule = GetDefaultRegRule(rotationTime)
	}

	// todo: glob pattern 无法正确处理 server.log / server.log.wf, 先做特殊处理
	p := basename + "." + regRule
	globPattern := basename + ".[0-9]*"
	...
	// 启动前检查是否已有未切割的日志
	filename := fileutil.GenerateFn(pattern, clock, rotationTime)
	if fi, err := os.Lstat(basename); err == nil {
		if mode := fi.Mode(); mode.IsRegular() {
			// 未开启切割之前的日志进行重命名
			if err := os.Rename(basename, filename); err != nil {
				return nil, errors.New(basename + "'s content might lack")
			}
		}
	}
...
func GetDefaultRegRule(rotationTime time.Duration) (p string) {
	if rotationTime < time.Hour {
		p = "%Y%m%d%H%M"
	} else if rotationTime < 24*time.Hour {
		p = "%Y%m%d%H"
	} else {
		p = "%Y%m%d"
	}
